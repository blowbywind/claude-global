...
---
[2026-07-08] hnedu_erp 패널 드래그 제스처 작업 완료 보고 / 응답 전체를 영문으로 작성 / **"한국어만" 규칙은 일본어·중국어뿐 아니라 영문 전체 응답에도 적용**
- **상황**: 좌/우 패널 드래그-숨김 기능 구현 완료 후 결과 요약을 전부 영어로 작성.
- **실수**: active-rules.md Rule 9가 "일본어·중국어 혼용 금지"로만 명시되어 있어 영문 전체 응답은 별도 금지로 인식 안 됨. 글로벌 CLAUDE.md는 "응답은 한글·영문·숫자·기본특수문자만"이라 명시했는데, 이를 "영문 단독 응답 허용"으로 잘못 해석함 — 실제 의도는 한국어 문장 안에 영문 용어·숫자가 섞이는 것을 허용한다는 뜻이지 영문 전체 응답 허용이 아님.
- **결과**: 사용자가 "영문으로 설명을 해서 작업 완료가 되었는지 확인이 안되고 있어"라며 재확인 요청 — 완료 보고가 무의미해짐.
- **교훈**: 사용자가 한국어로 대화 중이면 상태 메시지·완료 보고·설명 전부 한국어 산문으로 작성. 코드 주석·커밋 메시지만 예외(영어). 코드 식별자·기술 용어는 문장 안에 섞어도 되지만 문장 자체는 한국어여야 함.
- **승격**: → `active-rules.md` Rule 9 문구에 "영문 전체 응답 금지" 명시 추가 완료 (2026-07-08)
...
[2026-06-24] hnedu_erp enum→text 전환 / **데이터 정합성 변경의 일괄 편집은 항목별 의미 확인 + 배포 전 adversarial review**
- **상황**: ERP 전역 PG enum 컬럼을 text로 전환하며 AppDbContext의 `HasColumnType("*_enum")` 27곳을 일괄 제거.
- **실수**: `HasColumnType`만 떼려 했는데 같은 줄의 `HasConversion<string>()`까지 함께 제거. employees 5개는 CLR이 `string`이라 무해했으나, 나머지 26개는 **C# enum 타입** → 변환기 없으면 EF가 **정수 ordinal("0"/"1")로 저장** → DB 오염 + `WHERE status='ACTIVE'` 부분인덱스 무력화. 빌드·단위(287)·통합 일부(5개) 통과로 **거짓 양성**, 운영 배포 직전까지 못 잡음. code-reviewer가 EF 모델 런타임 프로빙으로 적발.
- **교훈**: ① enum 매핑 정리 시 CLR 타입(string vs C# enum)을 먼저 구분 — string만 text 매핑으로 충분, C# enum은 `HasConversion<string>()` 유지 필요(전역 `ConfigureConventions Properties<Enum>().HaveConversion<string>()`가 누락 없는 정답). ② **sed류 광범위 기계 편집은 "한 줄에 의미가 다른 토큰이 섞여 있지 않은지" 항목별 확인**. ③ 데이터 정합성/스키마 영향 변경은 **배포 전 code-reviewer 게이트 필수**, 통합테스트 프로빙이 일부만 검증하면 거짓 양성 가능 → round-trip 값 검증.
- **승격**: → `active-rules.md` Rule 11 추가 완료 (2026-06-24).
...
[2026-06-11] 응답 언어 혼용 / 일본어 출력 / **한국어 사용자에게 일본어 절대 금지**
- **상황**: 도구 호출 직전 상태 메시지를 일본어("全体を確認して完全な置換を行います。")로 출력
- **교훈**: 사용자가 한국어로 대화 중이면 모든 텍스트 출력(상태 메시지·주석·설명)을 한국어로 작성한다. 일본어·중국어 혼용 절대 금지.
- **승격**: → `active-rules.md` Rule 9 추가 (번호 드리프트로 정정, 2026-06-24)
...
[2026-06-18] 파이프라인 성공 — [#admin] 관리자 채널 테스트 이벤트: 서버 배포 완료 확인 요청
봇: 해리 / 런타임: claude
요약: ---
